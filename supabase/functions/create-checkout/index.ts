...
serve(async (req) => {
  ...
  try {
    ...
    if (isStage1Format) {
      ...
      const finalSuccessUrl = successUrl ?? `${baseOrigin}/report?guest_id=${guest_report_id}&status=success&session_id={CHECKOUT_SESSION_ID}`;
      const finalCancelUrl = cancelUrl ?? `${baseOrigin}/report?guest_id=${guest_report_id}&status=cancelled`;
