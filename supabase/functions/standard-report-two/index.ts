    ...
    try {
      ...
      if (reportData.user_id && reportData.is_guest) {
        try {
          ...
          if (insertData?.id) {
            console.log(`[standard-report-two][${requestId}] Calling finalize-report for guest report:`, {
            ...
            if (!finalizeResponse.ok) {
              ...
              console.error(`[standard-report-two][${requestId}] finalize-report call failed:`, {
                status: finalizeResponse.status,
                error: errorText
              });
            } else {
              const finalizeResult = await finalizeResponse.json();
              console.log(`[standard-report-two][${requestId}] finalize-report completed successfully:`, finalizeResult);
            }
          } else {
            console.error(`[standard-report-two][${requestId}] Could not find report_log_id for guest report`);
          }
        } catch (finalizeError) {
          console.error(`[standard-report-two][${requestId}] Error calling finalize-report:`, finalizeError);
        }
      }
    } catch (logError) {
