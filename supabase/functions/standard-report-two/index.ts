    ...
    try {
      ...
      if (reportData.user_id && reportData.is_guest) {
        try {
          ...
          if (insertData?.id) {
            console.log(`[standard-report-two][${requestId}] Calling link-report-guest for guest report:`, {
            ...
            EdgeRuntime.waitUntil(
              fetch(`${SUPABASE_URL}/functions/v1/link-report-guest`, {
                ...
                console.error(`[standard-report-two][${requestId}] link-report-guest call failed:`, error);
              })
            );
            
            console.log(`[standard-report-two][${requestId}] link-report-guest called asynchronously`);
          } else {
            console.error(`[standard-report-two][${requestId}] Could not find report_log_id for guest report`);
          }
        } catch (guestError) {
          console.error(`[standard-report-two][${requestId}] Error calling link-report-guest:`, guestError);
        }
      }
    } catch (logError) {
