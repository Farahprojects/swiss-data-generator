...
Ensures an object has latitude & longitude.
Returns the (possibly enriched) object PLUS a flag saying
whether the Google Geocoding API was actually called. / async function ensureLatLon( obj: any, ): Promise<{ data: any; googleGeoUsed: boolean }> { / Already geocoded or no location supplied */ if ( (obj.latitude !== undefined && obj.longitude !== undefined) || !obj.location ) { return { data: obj, googleGeoUsed: false }; }
...
/* ---------- fall back to Google ---------- */
const url =
https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(       place,     )}&key=${GEO_KEY};
...
let googleGeoUsed = false; // aggregate flag for the whole request
...
if (canon === "moonphases") {
  ...
  await logToSupabase(
    ...
    Date.now() - startTime,
    !r.ok ? `Swiss API returned ${r.status}` : undefined,
    googleGeoUsed,   // always false here
  );
