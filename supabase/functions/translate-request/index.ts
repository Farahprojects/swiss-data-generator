...
import { handleReportGeneration } from "../_shared/reportHandler.ts";
...
const TRANSLATOR_VERSION = "2025-07-21-edge-function";
...
async function logToSupabase(
  ...
  console.log('[translate-request] [DEBUG] responsePayload keys before saving:', Object.keys(responsePayload || {}));
  console.log('[translate-request] [DEBUG] responsePayload.swiss_data exists:', !!responsePayload?.swiss_data);
  console.log('[translate-request] [DEBUG] Swiss data keys being saved:', responsePayload?.swiss_data ? Object.keys(responsePayload.swiss_data) : 'null');
  
  console.log('⭐ [TRANSLATE-REQUEST] insert', {
    user_id: userId,
  ...
  const insertData = {
    ...
    swiss_data:          responsePayload ?? null,
    processing_time_ms:  processingTime,
    error_message:       errorMessage,
    google_geo:          googleGeoUsed,
    report_tier:         reportTier,
    user_id: isValidUUID(userId) ? (userId ?? '').toString().trim() : null,
    is_guest:            isGuest,
  };
  ...
  if (error) {
    console.error("⭐ [TRANSLATE-REQUEST] insert_failed:", error.message);
  } else {
    console.log('⭐ [TRANSLATE-REQUEST] insert_success', { 
      user_id: userId,
      user_id_type: typeof userId,
      function: "translate-request"
    });
...
export async function translate(
  ...
  console.log(`[translate-request][${requestId}] ✅ TRANSLATOR VERSION: ${TRANSLATOR_VERSION} - EDGE FUNCTION`);
  ...
  try {
    ...
    if (requestType === "report" || requestType === "reports") {
      ...
      console.warn(`[translate-request][${requestId}] ${err}`);
      return { status: 400, text: JSON.stringify({ error: err }) };
    }
    ...
    if (canon === "sync") {
      ...
      console.log(`[translate-request][${requestId}] POST /sync`);
      ...
      console.log(`[translate-request][${requestId}] [DEBUG] finalData keys after handleReportGeneration:`, Object.keys(finalData || {}));
      console.log(`[translate-request][${requestId}] [DEBUG] finalData.swiss_data exists:`, !!finalData?.swiss_data);
    ...
    if (canon === "moonphases") {
      ...
      console.log(`[translate-request][${requestId}] GET /moonphases`);
      ...
      if (raw.report) {
        console.log(`[translate-request][${requestId}] [REPORT-HANDLER-MOONPHASES] Processing report data in moonphases`);
        console.log(`[translate-request][${requestId}] [REPORT-HANDLER-MOONPHASES] Raw swiss response preview:`, txt.substring(0, 200));
        ...
        console.log(`[translate-request][${requestId}] No report field found, returning Swiss API response directly`);
        return {
          status: r.status,
          text: txt,
        };
    ...
    if (canon === "positions") {
      ...
      console.log(`[translate-request][${requestId}] GET /positions`);
      ...
      if (raw.report) {
        console.log(`[translate-request][${requestId}] [REPORT-HANDLER-POSITIONS] Processing report data in positions`);
        console.log(`[translate-request][${requestId}] [REPORT-HANDLER-POSITIONS] Raw swiss response preview:`, txt.substring(0, 200));
    ...
    const path    = canon;
    console.log(`[translate-request][${requestId}] POST /${path}`);
    ...
    if (raw.report) {
      console.log(`[translate-request][${requestId}] [REPORT-HANDLER-CHARTS] Processing report data in chart routes (${path})`);
      console.log(`[translate-request][${requestId}] [REPORT-HANDLER-CHARTS] Raw swiss response preview:`, txt.substring(0, 200));
      const reportResult = await handleReportGeneration({
        requestData: raw,
        swissApiResponse: txt,
        swissApiStatus: r.status,
    ...
    console.log(`[translate-request][${requestId}] ❌ ERROR: ${msg}`);
    if (!skipLogging) {
      await logToSupabase(
        requestType,
        raw,
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[translate-request][${requestId}] Processing request:`, {
