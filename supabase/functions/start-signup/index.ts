...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch the verification token that was generated by the trigger
    ...
    if (profileError || !profileData?.verification_token) {
      console.error('Error fetching verification token:', profileError);
      return new Response(
        JSON.stringify({ 
          success: false,
          error: 'Failed to retrieve verification token',
    ...
    console.log('Verification token retrieved:', profileData.verification_token);
    ...
    const { error: tokenError } = await supabase.functions.invoke('signup_token', {
      body: {
        ...
        verification_token: profileData.verification_token
      }
    });
