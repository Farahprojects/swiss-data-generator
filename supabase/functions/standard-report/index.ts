...
serve(async (req) => {
  ...
  try {
    ...
    .then(() => console.log(`[standard-report][${requestId}] Report log insert succeeded for ${reportData.is_guest ? 'guest' : 'user'} report`))
    .catch(err => console.error(`[standard-report][${requestId}] Report log insert failed:`, {
      error: err,
      user_id: reportData.user_id,
      is_guest: reportData.is_guest,
      report_type: reportType
    ...
    if (reportData.is_guest && reportData.user_id) {
      ...
      .then(() => console.log(`[standard-report][${requestId}] Signal inserted for guest report: ${reportData.user_id}`))
      .catch(err => console.error(`[standard-report][${requestId}] Signal insert failed:`, err));
    }
    ...
    .then(() => console.log(`[standard-report][${requestId}] Error report log insert succeeded`))
    .catch(err => console.error(`[standard-report][${requestId}] Error report log insert failed:`, {
      report_type: reportData?.reportType || reportData?.report_type,
      user_id: reportData?.user_id,
      error: err
    }));
