  ...
  try {
    ...
    try {
      ...
      if (insertLog.error) {
        ...
        console.error(`[standard-report][${requestId}] Report log insert failed:`, insertLog.error);
      } else {
        // ✅ LOGGING: Report log insert successful
        console.log(`[standard-report][${requestId}] Report log insert successful:`, {
          report_type: reportType,
          user_id: reportData.user_id,
        ...
        if (reportData.user_id && reportData.is_guest && insertLog.data?.[0]?.id) {
          try {
            ...
            if (guestUpdateError) {
              ...
              console.error(`[standard-report][${requestId}] Guest report linking failed:`, {
                report_type: reportType,
                user_id: reportData.user_id,
                report_log_id: insertLog.data[0].id,
              ...
              console.log(`[standard-report][${requestId}] Guest report linking successful:`, {
                report_type: reportType,
                user_id: reportData.user_id,
                report_log_id: insertLog.data[0].id
              });
            ...
            console.error(`[standard-report][${requestId}] Guest report linking exception:`, {
              report_type: reportType,
              user_id: reportData.user_id,
              report_log_id: insertLog.data[0].id,
              error: guestError
