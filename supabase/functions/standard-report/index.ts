    ...
    try {
      ...
      if (reportData.user_id && reportData.is_guest) {
        try {
          ...
          if (insertData?.id) {
            console.log(`[standard-report][${requestId}] Calling link-report-guest for guest report:`, {
            ...
            EdgeRuntime.waitUntil(
              fetch(`${SUPABASE_URL}/functions/v1/link-report-guest`, {
                ...
                console.error(`[standard-report][${requestId}] link-report-guest call failed:`, error);
              })
            );
            
            console.log(`[standard-report][${requestId}] link-report-guest called asynchronously`);
          } else {
            console.error(`[standard-report][${requestId}] Could not find report_log_id for guest report`);
          }
        } catch (guestError) {
          console.error(`[standard-report][${requestId}] Error calling link-report-guest:`, guestError);
        }
      }
    } catch (logError) {
