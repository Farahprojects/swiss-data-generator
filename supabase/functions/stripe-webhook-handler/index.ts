...
/* ========================================================================== *
   ...
   Notes   : • "credited" column is NOT set here; DB trigger flips it to true
             • Duplicate log prevention: we upsert once per PaymentIntent
...
async function logTopupSuccess(userId: string, pi: Stripe.PaymentIntent) {
  ...
  const { error: insErr } = await supabase.from("topup_logs").insert({
    ...
    // credited left to default false; DB trigger will flip it
  });
...
serve(async (req) => {
  ...
  try {
    switch (evt.type) {
      ...
      /* charge.succeeded may fire alongside payment_intent.succeeded.
         We ONLY log if, for some edge case, the PaymentIntent hasn't been stored. */
