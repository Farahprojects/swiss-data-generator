...
/* ========================================================================== *
   ...
   Notes   : • "credited" column is NOT set here; DB trigger flips it to true
             • Duplicate log prevention: upsert once per PaymentIntent
             • receipt_url pulled reliably (expands / charge fetch fallback)
...
async function logTopupSuccess(userId: string, pi: Stripe.PaymentIntent) {
  /* prevent duplicates — quick look-up by PaymentIntent */
  ...
  /* 1️⃣ try inline charge first */
  let receiptUrl: string | null = pi.charges?.data?.[0]?.receipt_url ?? null;

  /* 2️⃣ fallback: fetch latest full charge if needed */
  if (!receiptUrl && pi.latest_charge) {
    const charge = await stripe.charges.retrieve(pi.latest_charge as string);
    receiptUrl = (charge as Stripe.Charge).receipt_url ?? null;
  }

  const { error: insErr } = await supabase.from("topup_logs").insert({
    user_id: userId,
    stripe_payment_intent_id: pi.id,
    amount_cents: pi.amount,
    status: "completed",
    receipt_url: receiptUrl, // may still be null
  ...
  if (topupRequestId) {
    ...
    if (updErr) console.error(`Failed to update topup_queue ${topupRequestId}:`, updErr);
  }
}
  ...
  try {
    switch (evt.type) {
      ...
      case "payment_intent.succeeded": {
        ...
        /* For consistency, retrieve a fresh PI with expanded charges */
        const pi = await stripe.paymentIntents.retrieve(piRaw.id, { expand: ["charges"] });
      ...
      /* charge.succeeded sometimes fires as well — log only if not done */
      case "charge.succeeded": {
        ...
        if (ch.payment_intent) {
          const pi = await stripe.paymentIntents.retrieve(
            ...
            { expand: ["charges"] }, // ensures receipt_url inline
          );
          await logTopupSuccess(userId, pi as Stripe.PaymentIntent);
        }
        break;
