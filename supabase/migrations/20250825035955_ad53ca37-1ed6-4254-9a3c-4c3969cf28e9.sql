...
DROP POLICY IF EXISTS "Users can insert to own chats" ON public.messages;
DROP POLICY IF EXISTS "Users can update own chat messages" ON public.messages;
...
CREATE POLICY "Authenticated users insert to own chats"
ON public.messages
FOR INSERT
TO authenticated
WITH CHECK (
