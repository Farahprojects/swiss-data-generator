...
interface UnifiedSuccessHandlerProps {
  name: string;
  email: string;
  guestReportId: string;
}

export const UnifiedSuccessHandler: React.FC<UnifiedSuccessHandlerProps> = ({
  name,
  email,
  guestReportId
}) => {
  ...
  useEffect(() => {
    const componentId = 'UnifiedSuccessHandler';
    
    const stateResetCallback = () => {
      console.log('🔄 UnifiedSuccessHandler: State reset triggered by SessionManager');
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          console.error('Error polling report_ready_signals', { error });
          return;
        }
