...
export const ReportViewer = ({ mappedReport, onBack, isMobile = false }: ReportViewerProps) => {
  ...
  const handleChatGPTCopyAndGo = async () => {
    // Production-grade fix: If token is cached, immediately copy and open ChatGPT
    if (chatToken && cachedUuid) {
      try {
        const tokenMessage = `uuid: ${cachedUuid}\ntoken: ${chatToken}`;
        await navigator.clipboard.writeText(tokenMessage);
        ...
        const message = encodeURIComponent(tokenMessage);
        ...
        toast({
          ...
          description: "Opening ChatGPT - you'll need to manually paste the token.",
          variant: "destructive"
        });
    ...
    try {
      ...
      /* 3. Copy token to clipboard automatically */
      const tokenMessage = `uuid: ${uuid}\ntoken: ${token}`;
      try {
        await navigator.clipboard.writeText(tokenMessage);
        toast({
          title: "✅ Copied to clipboard!",
        ...
        toast({
          title: "Clipboard access failed",
          description: "You'll need to manually copy the token from ChatGPT.",
          variant: "destructive"
        });
      }

      /* 4. Open ChatGPT with pre-filled message */
      const message = encodeURIComponent(tokenMessage);
      const gptUrl =
